...
const Num = x => new ToleranceNumber(x);
...
const Seg = (p1,p2) => new LineSegment(p1,p2);
const Line = (p1,p2) => new LineModel(p1,p2);
const Rect = (p1,p2) => new Rectangle(p1,p2);
const Poly = points => new Polygonal(points);
const Vec = (origin, vec) => new Vector(origin, vec);
const Size = (width, height) => new SizeModel(width, height);
const Func = func => new FunctionGraph(func);
const Origin = Pt(0,0);
